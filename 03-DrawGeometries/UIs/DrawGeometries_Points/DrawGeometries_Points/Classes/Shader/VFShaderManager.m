 ...
 *  解析着色器代码的变量
 */
- (void)analyzerShaderCodeWithFileName:(NSString *)fileName shaderType:(VFShaderType)shaderType {
    
    [self.shaderCodeAnalyzer analyzingShaderCodeWithFileName:fileName
                                                  shaderType:(shaderType == GL_VERTEX_SHADER ? VERTEX_SHADER :
                                                                                               FRAGMENT_SHADER)];
...
- (GLuint)loadShaderCodeWithFileName:(NSString *)fileName shaderType:(VFShaderType)shaderType {
    ...
    [self analyzerShaderCodeWithFileName:fileName shaderType:shaderType];
 ...
 */
- (void)bindShaderAttributeValuesWithShaderProgramID:(GLuint)programID {
    
//    glBindAttribLocation(programID, VFVertexPositionAttribute, "v_Position");
//    glBindAttribLocation(programID, VFVertexColorAttribute, "v_Color");
    ...
    [attributes enumerateKeysAndObjectsUsingBlock:^(VFValueNameString * _Nonnull key, VFShaderValueInfo * _Nonnull info, BOOL * _Nonnull stop) {
        glBindAttribLocation(programID, (GLint)info.location, [info.name UTF8String]);
    }];
